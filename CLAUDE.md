...
| **A published result whose code is being replaced** | **P** — published model, reimplemented |
...
rewritten in Python, an R reporting layer rewritten in Python, or a folder of loose scripts
being packaged.

#### The rule: one working language, and the original is archived rather than maintained

> **Everything that produced the published result is preserved verbatim. Exactly one
> implementation is maintained, and it is Python.**
...
to the figures. Port the model *and* the reporting; archive both originals.
    ...
    a test                        fails if an archived file changes

**"Archived" is not "deleted".** The original is the only evidence of what produced the
published numbers, and it is often the only thing that can still be run against them. Keep it
in the repository, not in a folder somewhere.

**"Archived" is also not "maintained".** Do not fix bugs in it. A correction goes in the
Python implementation and the divergence gets recorded, because the original's job is to say
what the paper did, not to be right.

#### Nine stages, one direction

Archetype A's three data tiers become nine named stages, because this archetype has two of
everything and "input, interim, output" stops distinguishing them.

    input raw            whatever the original read, wherever it lives
    clean-up code        may require the original's tool; runs once
    cleaned data         open, plain-text. COMMITTED - see below
    model files          the archived original AND the maintained implementation
...
already reads open plain-text there is no clean-up code and no separate cleaned tier; the
stage exists to be named as absent rather than to be manufactured.

**Two of these are committed generated files, and both are deliberate exceptions to Part 1
rule 5.** Cleaned *data* is committed because regenerating it may require the tool it exists
to remove — the usual reasoning runs backwards. Cleaned *output* is committed because without
it nothing in the repository works from a clean clone. Verify that by cloning and running,
not by reasoning: in one repo the suite passed on a clone that had no data at all, because
notebooks are not executed by the suite.

**Figures read cleaned output, never raw.** A figure that parses the original tool's report
is re-implementing the clean-up stage, and the second copy is the one that drifts. One such
format had already dropped 19% of a comparison silently, by carrying a *varying* number of
index columns where a reader assumed a fixed one.

#### Four artifacts, and what each may claim

| | needs | claims |
|---|---|---|
| the archived original | its own tool | produced the published result |
| the implementation | a solver | reproduces it |
| verification notebook | **no solver, no licence** | the published result is correct |
| example notebook | a free solver | the implementation runs and behaves |

**Keep the two claims apart — the claims, not necessarily the files.** One notebook may
carry both if the sections are separated, but the verification path must not *import* a
solver: a licence-free claim made in a file that fails at import for someone without a
solver is not licence-free. Two files is the simplest way to guarantee that; one file works
if the solver import is confined to the cells that need it. A reduced instance cannot reproduce a full-horizon result and must not be
presented as trying — stamp the reduction **into the image**, not into markdown beside it,
because a chart gets screenshotted into a slide and the caption does not follow it.

**Stamp a genuine subset. Do not stamp an exact reduction.** Smaller is not the same as
approximate, and marking an exact result "reduced" is a false warning. Where structure
collapses the instance without changing its answer — one model went from 704,002 variables
to 37 with integer weights, agreeing with the full model to $0.09 under a test that asserts
it — the figure is of the real result and should not carry a caveat it does not deserve.
...
variable, and demanding that manufactures failures. Degenerate problems have alternate
optima: in one measured case cost and capacity totals reproduced exactly while dispatch
differed by 9–22% on two variables that nothing constrained and nothing costed — the optimum
simply did not determine them.

**Where the original can still be run, reconcile against it. Where it cannot, reconcile
against the published numbers** and say which you did.
...
**A status is not a feasibility guarantee.** `OPTIMAL` has been observed alongside a primal
residual three orders above the solver's own promised tolerance. **Read the residual, not the
status word.**

What to do with it depends on the model, and the two measured remedies are different:

- **Scale the residual** by the largest constraint-matrix coefficient, where the model's
  quantities are commensurate. Measured on a model whose matrix spans `[1e-06, 1e+06]`: a
  residual of 2.1e-07 absolute is 1.95e-13 relative, i.e. machine precision.
- **Do not scale where a small physical quantity feeds a nonlinear response.** A tolerance
  that is negligible against a $2.2M objective can be enormous against a 0.022 cm water
  balance that drives a yield curve — scaled at 1e-8 it would have admitted ~$1,300 of error
  on quantities asserted to $2. There, **clip the returned point back inside the feasible
  region and re-price it**, which makes the reported value feasible by construction and a
  lower bound. Measured cost of the repair: $1.48 per solve.

An **absolute** threshold is wrong in both cases: one such gate rejected 87 of 91 correct
solves whose relative violation was 4e-10.

#### When the reimplementation crosses languages

These apply only where the original is in another language. Where it is not — packaging loose
Python — skip this section; the rest of the archetype still holds.

**Grep the source for variable BOUNDS before believing a port is complete.** Walking the
equation list is not sufficient and cannot be: bounds are not equations. One model carried 107
equations and exactly one bound — `ProductionByTechnology.fx(...) = 0` among the variable
declarations — and omitting it left the objective 1.70 low on 72,412. Too small to notice, far
too large to be arithmetic, invisible to a check that diffs equations. Sweep `.fx` `.up` `.lo`
in GAMS, `setLB`/`setUB` elsewhere, and **scope the sweep to the files that produced the
published result** — one session's first attempt globbed a `superseded/` folder and reported
bounds from an abandoned variant.
...
fixed, moved that objective by exactly zero because the constraint was not binding. Finding it
nearly ended the search.
...
against the original's 18.76M for identical mathematics. **Do not report the two sizes as if
...
- **Shipping the model as `.mps`/`.sol` dies above roughly 2M nonzeros.** Flat formats run
  ...
  row by row; above it, reconstruct the objective from its cost components instead — weaker as
  proof, better as explanation. One model's would have been 3.4 GB.
- **"Runs for everyone" requires a free solver for the problem class.** True for LP, marginal
  for MIP, false for MINLP. A size-limited licence is **not** the fallback: one vendor's
  bundled licence caps at 2,000 variables where the smallest useful reduction of a real model
  was 4,726. Verify the cap by **solving**, not by declaring variables — that vendor enforces
  it at optimize, so a probe that only declares reports success under a licence that then
  refuses the real build.
- **Committed cleaned output assumes it stays small.** Around 10 MB is comfortable. Above
  that the pattern survives by **committing a different layer**, not by being abandoned: one
  repo's tidy per-run detail is 68 MB, and it commits the summary tables (a few KB) plus
  twelve frozen instances (164 KB) and regenerates the bulk. A clean clone still works,
  which is the property that actually matters. Compressing or subsetting is the other route.
- **The archived original is frozen only once the work is finished.** For a paper repo that is
  right, and a test should enforce it. For active research the original *is* the working copy
  and freezing it is wrong.
- **Presentation choices do not travel.** Splitting an objective into named parts helps where
  one summation spans millions of terms and hurts everywhere else. Mark such choices as local
  when you make them.
