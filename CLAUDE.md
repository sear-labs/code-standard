...
claim and needs no solver; the example notebook demonstrates the code on whatever it can
actually solve. A reduced instance cannot reproduce a full-horizon result and must not be
...
residual three orders above the solver's own promised tolerance. Read the residual, and scale
it by the largest constraint-matrix coefficient — an absolute threshold is wrong for a model
whose coefficients span orders of magnitude, and one such gate rejected 87 of 91 correct
solves.
